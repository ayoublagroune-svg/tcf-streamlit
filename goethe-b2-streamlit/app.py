...
APP_TITLE = "Goethe-Zertifikat B2 Trainer"
...
def render_home(questions):
    st.title(APP_TITLE)
    st.markdown('<span class="badge">Goethe-Zertifikat B2</span>', unsafe_allow_html=True)
    st.write("Trainieren Sie mit B2-Aufgaben für Lesen, Hören, Schreiben und Sprechen. KI-Erklärungen liefern Bedeutung, Sprachregeln und Tipps auf Deutsch und Arabisch.")
...
def answer_explanation_prompt(item):
    choices = "\n".join(f"{key}. {value}" for key, value in item["choices"].items())
    user_answer = item.get("user_answer") or "Keine Antwort"
    return f"""
Du bist Sprachcoach für Goethe-Zertifikat B2.
Erkläre diese Aufgabe zweisprachig: zuerst Deutsch, danach Arabisch in klassischem Arabisch.
Schreibe klar, prüfungsnah und nicht zu lang.
...
Antwort des Kandidaten: {user_answer}
Richtige Antwort: {item['answer']} - {item['choices'][item['answer']]}

Gib:
1. Bedeutung der richtigen Antwort: Was sagt sie in einfachen Worten?
2. Warum ist sie richtig?
3. Warum passen die anderen Optionen nicht?
4. Sprachregel oder Prüfungsstrategie, die man hier lernen soll.
5. Zwei bis drei konkrete Tipps für ähnliche Goethe-B2-Aufgaben.
...
def writing_feedback_prompt(task, answer):
    return f"""
Du bist Prüfer und Sprachcoach für Goethe-Zertifikat B2 Schreiben.
Antworte zweisprachig: zuerst Deutsch, danach Arabisch in klassischem Arabisch.
Bewerte nicht offiziell, sondern gib präzises Training-Feedback.
...
Gib:
1. geschätztes Niveau und kurze Begründung,
2. Inhalt: was die Antwort bedeutet und ob die Aufgabe erfüllt ist,
3. wichtigste Sprachregeln, die der Kandidat braucht,
4. typische Fehler mit Korrekturen,
5. verbesserte B2-Version,
6. drei konkrete Tipps für die nächste Produktion.
Nutze klare Überschriften: Deutsch / العربية. Maximal 500 Wörter.
"""


def render_writing_module(tasks, expired):
    st.info("Goethe B2 Schreiben: Teil 1 Forumsbeitrag mit Meinung und Begründung, Teil 2 formelle Nachricht im beruflichen Kontext.")
    for task in tasks:
        with st.container(border=True):
...
def render_speaking_module(tasks, expired):
    st.info("Goethe B2 Sprechen: kurzer Vortrag mit Nachfragen und Diskussion mit Argumentenaustausch.")
    for task in tasks:
        with st.container(border=True):
...
def render_dashboard(score):
    ...
    st.caption("Goethe B2 ist modular. In der echten Prüfung wird jedes Modul separat bewertet; diese App ist ein Training.")
...
def render_correction(questions):
    ...
    for index, item in enumerate(score["items"], start=1):
        ...
        with st.expander(f"Aufgabe {index} - {status} - {item['theme'].replace('_', ' ')}", expanded=not item["correct"]):
            ...
            st.caption("Die lokale Kurzkorrektur bleibt als Fallback verfügbar. Die ausführliche Erklärung kommt aus dem KI-Modus.")
            with st.container(border=True):
                st.write("**Lokale Kurzkorrektur**")
                st.write(f"**Erklärung:** {item['explanation']}")
                st.write(f"**Strategie:** {item['strategy']}")
            ai_key = f"answer_explanation_{item['id']}"
            if st.button("KI-Erklärung: Bedeutung, Regel und Tipps", key=f"ai_explain_{item['id']}"):
                with st.spinner("KI-Erklärung wird erzeugt..."):
                    st.session_state.ai_outputs[ai_key] = call_ai(answer_explanation_prompt(item), max_tokens=1200)
            if ai_key in st.session_state.ai_outputs:
                st.markdown(st.session_state.ai_outputs[ai_key])
    ...
    with c1:
        st.download_button("PDF", pdf_bytes(payload), "goethe_b2_report.pdf", "application/pdf", use_container_width=True)
    with c2:
        st.download_button("CSV", csv_bytes(payload), "goethe_b2_report.csv", "text/csv", use_container_width=True)
    with c3:
        st.download_button("JSON", json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8"), "goethe_b2_report.json", "application/json", use_container_width=True)
...
def render_about():
    st.title("Prüfungsformat Goethe B2")
    st.write("Diese App orientiert sich am offiziellen Aufbau des Goethe-Zertifikats B2, verwendet aber eigene Trainingsaufgaben.")
    st.write("- Lesen: 5 Teile, 65 Minuten, ungefähr 30 Items")
    st.write("- Hören: 4 Teile, 40 Minuten, ungefähr 30 Items")
    st.write("- Schreiben: 2 Aufgaben, 75 Minuten")
    st.write("- Sprechen: 2 Aufgaben, circa 15 Minuten plus Vorbereitung")
    st.write("- KI-Modus: optionale Erklärungen zu Bedeutung, Sprachregel und Tipps auf Deutsch und Arabisch")
...
def main():
    st.set_page_config(page_title=APP_TITLE, page_icon="B2", layout="wide")
    init_state()
    render_header()
    render_ai_controls()
